...
static inline int32x4_t cactus_dotq_lane0_fallback(int32x4_t acc, int8x16_t b, int8x16_t a) {
...
static inline int32x4_t cactus_dotq_lane1_fallback(int32x4_t acc, int8x16_t b, int8x16_t a) {
    int8x8_t a_lo = vget_low_s8(a);
    int8x8_t a_pattern = vreinterpret_s8_s32(vdup_lane_s32(vreinterpret_s32_s8(a_lo), 1));
    return cactus_dotq_with_pattern(acc, b, a_pattern);
}

static inline int32x4_t cactus_dotq_lane2_fallback(int32x4_t acc, int8x16_t b, int8x16_t a) {
    int8x8_t a_hi = vget_high_s8(a);
    int8x8_t a_pattern = vreinterpret_s8_s32(vdup_lane_s32(vreinterpret_s32_s8(a_hi), 0));
    return cactus_dotq_with_pattern(acc, b, a_pattern);
}

static inline int32x4_t cactus_dotq_lane3_fallback(int32x4_t acc, int8x16_t b, int8x16_t a) {
    int8x8_t a_hi = vget_high_s8(a);
...
static inline bool cactus_runtime_use_dotprod() {
    // On Linux/Android this reflects actual CPU support; on other targets it
    // intentionally preserves existing always-on DOTPROD behavior.
    static const bool has_dotprod = cpu_has_dotprod();
    return has_dotprod;
}

static inline int32x4_t cactus_dotq_lane0_dotprod(int32x4_t acc, int8x16_t b, int8x16_t a) {
    return vdotq_laneq_s32(acc, b, a, 0);
}

static inline int32x4_t cactus_dotq_lane1_dotprod(int32x4_t acc, int8x16_t b, int8x16_t a) {
    return vdotq_laneq_s32(acc, b, a, 1);
}

static inline int32x4_t cactus_dotq_lane2_dotprod(int32x4_t acc, int8x16_t b, int8x16_t a) {
    return vdotq_laneq_s32(acc, b, a, 2);
}

static inline int32x4_t cactus_dotq_lane3_dotprod(int32x4_t acc, int8x16_t b, int8x16_t a) {
    return vdotq_laneq_s32(acc, b, a, 3);
}
#endif

static inline int32x4_t cactus_dotq_lane0(int32x4_t acc, int8x16_t b, int8x16_t a) {
#if defined(__ARM_FEATURE_DOTPROD)
    if (cactus_runtime_use_dotprod()) {
        return cactus_dotq_lane0_dotprod(acc, b, a);
    }
#endif
    return cactus_dotq_lane0_fallback(acc, b, a);
}

static inline int32x4_t cactus_dotq_lane1(int32x4_t acc, int8x16_t b, int8x16_t a) {
#if defined(__ARM_FEATURE_DOTPROD)
    if (cactus_runtime_use_dotprod()) {
        return cactus_dotq_lane1_dotprod(acc, b, a);
    }
#endif
    return cactus_dotq_lane1_fallback(acc, b, a);
}

static inline int32x4_t cactus_dotq_lane2(int32x4_t acc, int8x16_t b, int8x16_t a) {
#if defined(__ARM_FEATURE_DOTPROD)
    if (cactus_runtime_use_dotprod()) {
        return cactus_dotq_lane2_dotprod(acc, b, a);
    }
#endif
    return cactus_dotq_lane2_fallback(acc, b, a);
}

static inline int32x4_t cactus_dotq_lane3(int32x4_t acc, int8x16_t b, int8x16_t a) {
#if defined(__ARM_FEATURE_DOTPROD)
    if (cactus_runtime_use_dotprod()) {
        return cactus_dotq_lane3_dotprod(acc, b, a);
    }
#endif
    return cactus_dotq_lane3_fallback(acc, b, a);
}
